...
Many of them, including the named three, can be rolled with \texttt{Advantage} or \texttt{Disadvantage}. 
...
If the save fails, the \texttt{Injury} is still tended, but the target knight makes a \texttt{Vigor} save. 
...
To track those injuries, use \texttt{Injury Slots} that record the nature of the \texttt{Injury}, the damage die that caused it, and its \texttt{Consequence}.
...
\begin{center}
  ...
  \label{injury_cons}
    \begin{tabular}{|c|p{6.4cm}|}
        \hline
        \textbf{Roll d4} & \textbf{Afflication} \\
        \hline
...
\begin{itemize}[nosep]
 ...
 In combat, it immobilizes enemies on the round of casting and impairs weapons until escape(\texttt{Vigor} check) on later turns.
