...
contents={%
 ...
 \ifodd\value{page}
    \includegraphics[width=\paperwidth,height=\paperheight]{media/background_handmade_flipped.png}
  \else
    \includegraphics[width=\paperwidth,height=\paperheight]{media/background_handmade.png}
  \fi}\fi}}
...
\begin{enumerate}[nosep, leftmargin=*]
    ...
    \item Declare \texttt{Stress} gain, remove matching \texttt{High-} and \texttt{Lowrolls} roll additional dice.
    ...
    \item If the damage die is discarded - assign a new one, but \texttt{Gambits} can't be changed.
...
\subsection*{Injuries} A body of a knight is tough and trained since childhood, but it can endure only so much. 
...
\newpage
\section*{\centering \fontsize{35}{45}\selectfont Outside of combat}
\vspace*{0.4 cm}
...
Only a swift moment of safety is required to recover one's \texttt{Guard}. 

\subsection*{Short Rest}

Probably can restore 1 Virtue if the knight has 0.

However, to recover spent \texttt{Focus} and reduce acquired \texttt{Stress} at least an hour of rest is needed. 


\paragraph{Focus.} Yap yap, each short rest is less effective.

\paragraph{Stress.} Each time, half.

\subsection*{Long Rest}

\paragraph{Virture restoration}. Sustenance, Remedy, Stimulants.
...
\subsection*{\centering Quirks}

\paragraph{Bloodlust.} A visceral reaction to blood fuels strength, but lessens control. 
You can upface one die of your weapon if you are wounded. However, if somebody is wounded, you can't engage them with nonlethal force
...
You ignore first damage to your \texttt{Vigor} per combat if it is not larger than \texttt{1}. 
...
However, each time you roll 20 on `Vigor` check you lose 1 `Vigor`
...
You can upface any die of that weapon. However, each time you strike, you must strike with the weapon you are obsessed with. 
...
\paragraph{Bloodthirsty.} Restore one \texttt{Focus} each time your hit reduces both \texttt{Vigor} and \texttt{Guard} with \texttt{Attack} you participate in. 
However, gain one \texttt{Stress} when an attack you participate in does not reduce \texttt{Vigor}. If this point of \texttt{Stress} would cause \texttt{Trauma} gain no \texttt{Stress}.


\end{multicols*}

\newpage
\section*{\centering \fontsize{35}{45}\selectfont Something to fill the gaps}
\vspace*{0.4 cm}
