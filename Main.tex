...
\section*{\centering Life in the Realm of ???}
\vspace*{0.4 cm}
...
\begin{itemize}[nosep]
    ...
    \item[\textbf{\textdagger}] If a knight lost all their \texttt{Vigor} or more through combat damage -- take a \texttt{Lethal Injury}.
\end{itemize}
...
No armour is worn, combat weapons are stored away.
...
\paragraph{Overnight Rest} also restores up to \texttt{3} missing dice.

\paragraph{Consuming Spur} refills \texttt{Spellweaving Pool} fully.
...
\begin{multicols*}{2}
    ...
    Your mind overloads with stress and instead of collapse it splits and now you are not one, but two.

    You create another knight that now they inhabit your body. 

    \begin{itemize}[nosep]
        \item Roll three \texttt{d12 + d6} and pick two of them and assign to \texttt{Clarity} and \texttt{Spirit}. 
        \item Determine \texttt{Focus} and \texttt{Stress} and track them separately.
        \item Decide if they are a \texttt{Spellweaver}, a \texttt{Witch} or neither. Pick a \texttt{Passion} and finally a name. 
    ...
    \end{itemize}
Once the splitting event is occurred, you switch to new personality immediately. 
...
\paragraph{Masochistic.} Pain is unbearable, but somehow it's a good thing. You regain all \texttt{Focus} each time you gain \texttt{Agony} \texttt{Affliction}.
However, don't recover \texttt{Focus} from \texttt{Short Rest}.
