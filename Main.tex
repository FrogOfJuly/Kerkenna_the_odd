...
\hrule \vspace{2mm}
\begin{center}
 Welcome another \emph{Into the Odd} hack that is tuned for the world of Kerkenna, a stranded archipelago 
 in the world of many other stranded archipelagos. With the addition of Magic, Injuries, and Quirks, it became its own system
...
\section*{\centering Life on the archipelago of Kerkenna}
\vspace*{0.4 cm}
...
Perform a roll and then remove a die from it that shares the number of faces with the one added. 
...
To roll with \texttt{Disadvantage} -- do the same, but you must remove the best one(least one for \texttt{Virtue Checks}, highest from \texttt{Attack Rolls}) from the roll. 
...
Then, determine the knight's capacity for \texttt{Injury}, \texttt{Focus} and \texttt{Stress} according to the steps below.
...
the amount of thought the knight can put into perfecting a task and the amount of effort they can put into a failed task to fix it.
...
If it does cause an Injury - fill the \texttt{Injury slot} of corresponding or greater severity, record damage die that dealt it and apply affliction(p. 8).
...
Victims are only Slain, Wounded or Injured if the Virtue was reduced by \texttt{Combat Damage}, never as a result of Virtue Loss.
...
Roll a damage die that did damage again and consult the table below.
...
\end{itemize}
If the knight has no \texttt{Focus} left, they can recover \texttt{1} during a \texttt{breather} by acquiring one \texttt{Stress}.

\subsection*{Short Rest}

To recover spent \texttt{Focus} and reduce acquired \texttt{Stress}, at least an hour of rest is needed. 
...
Each time a knight does an hour of \texttt{Short Rest}, they restore \texttt{Focus} up to their maximum minus the number of \texttt{Short Rests} they already had, including this one.
...
\paragraph{Stress} is either halved(rounded down) or reduced by \texttt{1} each time knights rests. You can pick each option only once per day.
...
Long rest in warm hospitality with a healthy meal removes \texttt{Stress} and restores \texttt{Virtues} and \texttt{Focus}.

\paragraph{Healing an Injury} removes it completely. This is a tricky endeavour which usually requires a professional to accomplish and comes with a price tag. 
...
Choose or make the Referee choose a \texttt{Quirk} for you that is described below and permanently add it to your knight. 
...
You ignore the first damage to your \texttt{Vigor} per combat if it is not larger than \texttt{2}. 
...
\paragraph{Masochistic.} Pain is unbearable, but somehow it's a good thing. You regain two \texttt{Focus} each time you receive a serious injury that is not from \texttt{Trauma}. 
...
\paragraph{Gore fascination.} You ignore \texttt{Distress} conditions of your \texttt{Injuries}. However, you must treat your \texttt{Injuries} by yourself. 
...
However, whenever you take a \texttt{Clarity} save during socialization, treat it as if it was opposed by one additional \texttt{Focus}.
...
There are many spells one can learn during their mage training, but regardless of syllabus, all mages know \textit{Detect Magic} by heart.
...
For the remaining combat, anyone who attacks the bearer of this curse can add a die from the picked ones to the dice that represent an attack before rolling them. The same die can be used by multiple attacks, but only a single hex can be cast upon any one creature.
...
Each time a hasted entity attacks, choose a weapon and add its \texttt{Weapon dice} to the \texttt{Attack} one additional time. 

\paragraph{Soothing Visions. } Pick one die with the same value rolled as the target's \texttt{Stress}. Reduce it by \texttt{1}.
...
\begin{itemize}[nosep]
 ...
 The more inconsequential something is, the more probable it is a shape of Prestidigitation.
    \item[\ding{108}] \textbf{Mending}, a minor repair spell. Can fix a tear in fabric, crack in leather, or a snapped thread.
\end{itemize}
...
\renewcommand\zinetitle{Kerkenna the Odd: Appendix}
...
\begin{itemize}[nosep]
    \item You don't lose Vigor. Since when you discover this change in yourself, record damage you receive that would have been subtracted from your \texttt{Vigor}.
    \item For each 3 points of damage accumulated this way treat your maximum \texttt{Vigor} as if it is reduced by \texttt{1}. When it reaches \texttt{0} your body loses structural integrity and dissolves to dust.
    \item You accumulate \texttt{Injuries} based on damage received and wither away when you are out of \texttt{Injury slots}.
...
\end{itemize}
If you find a puppet master that will agree to such outstanding demand, they could try to patch your body to restore its former shape.
They will not be able to heal you from the illness.
...
% Stored in a translucent ampule with nutritious goo it sleeps peacefully. 
% Until one day the craving soul unleashes it onto their own body. 

Parasite awakes and digs into flesh. 
After moment of -- pain its gone. Unnatural motions of muscles and skin are the only evidence of where it is. 
In a day it reaches brain and hatches. Victim wakes up and notices that they can sense more of the arcane that they were able before. 
...
\begin{itemize}[nosep]
    \item Each morning you must feed the parasite. After waking up -- roll a die from your \texttt{Spellweaving Pool}, lose that much \texttt{Clarity}. 
    \item Record total amount of \texttt{Clarity} lost this way. 
    For each \texttt{20} \texttt{Clarity} lose \texttt{1} \textit{maximum} \texttt{Clarity} and add an additional \texttt{d4} to the pool. 
    \texttt{Upface} it once for each other die in the pool.
...
% TODO: adds spellcasting in exchange for clarity. When you feed it enough, it cocoons and emerges more potent. 
% TODO: it might be showcased as faceness of spellcasting dice, more spells or the size of spellweaving pool
