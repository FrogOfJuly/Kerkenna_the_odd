...
\newpage
\section*{\centering Creating a Knight}
\vspace*{0.4 cm}
...
\newcolumn
\subsection*{Magic of Kerkenna}

Yap, yap, guild, yap, yap orders contract mages

\paragraph{A Mage.}

Choose spells, your dice pool is your clarity.

\paragraph{A Witch.} 

Choose Patron your patron has passions, indulge them -- get rewarded with favour.

\subsection*{Weapons \& Gear}

Yap, yap, swords. Has only two legs and two arms, can wear only one clothes per layer. Tell about Armour.

\subsection*{Passion \& Ability}

Come up with an idea how to reference Bastionland without actually referencing it.

\subsection*{Company}

\paragraph{Squires.} Yap yap


\end{multicols*}

\newpage
\section*{\centering Basic Rules}
\vspace*{0.4 cm}
...
Limits, penalties for 0s, Wounded status.
...
Make a new one, add to the company

\subsection*{Attacks}

Attacks always hit, so gather all your weapon dice -- those are your attack roll. 
At this moment the knight declares and spends \texttt{Focus}, target declares opposition and matching \texttt{High-} and \texttt{Lowrolls} are removed.

All combatants that attack the same target roll simultaneously, but do not mix the dice together as \texttt{Focus} and \texttt{Stress} is applied separately.

Remove extra dice added through \texttt{Focus}.

After that the \texttt{Stress} is acquired by attackers and their opponent, matching \texttt{High-} and \texttt{Lowrolls} are removed and extra dice are rolled.

Remove extra dice from \texttt{Stress}.

Now you can mix all dice together and declare which one is used as a damage die and which are spent on \texttt{Gambits}.

If opponent knows the \texttt{Deny} feat they use it after you decided. 

You can choose some other die as a damage die, but you can't reassign \texttt{Gambits}.

Order of attack

\paragraph{Impared.} d4

\paragraph{Blast.} d4

\subsection*{Damage.}

\paragraph{Taking a scar.}

Yap yap

\paragraph{Sustaining an Injury.}
...
\begin{itemize}[nosep]
    ...
    \item \textbf{Trap}
    \item \textbf{Dismount}
    \item \textbf{Patch a Wound}
    \item \textbf{Other effect}
\end{itemize}
...
\begin{itemize}[nosep]
    ...
    \item \textbf{Greater effect} such as disarming an item, breaking a wooden shield or weapon, or removing a helm. This does not increase the Damage from Bolster.
\end{itemize}
...
\newpage
\section*{\centering Lasting damage}
\vspace*{0.4 cm}
...
\subsection*{\centering Injury Quirks}

\paragraph{Bloodlust.} \lipsum[2]

\paragraph{Thickskinned.} \lipsum[2]

\paragraph{Desensitized.} \lipsum[2]

\paragraph{High pain tolerance.} \lipsum[2]

\paragraph{Desperate gasp.} \lipsum[2]

\paragraph{Blade obsession.} \lipsum[2]

\paragraph{Gore facination.} \lipsum[2]

\paragraph{Stressjuncky.} \lipsum[2]

\paragraph{Bloodthirsty.} \lipsum[2]

\newpage
\section*{\centering Appendix}
\vspace{0.4cm}

\subsection*{\centering Spells}
